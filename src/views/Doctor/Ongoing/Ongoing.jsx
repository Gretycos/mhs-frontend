...
const Ongoing = () => {
    ...
    const title = (data) => {
        return <span className="ongoing-card-title">{data.day}, {data.date}, {data.time}</span>
    }
    ...
    const renderCards = () => {
        ...
        return listdata.slice(startIndex, endIndex).map((data) => (
            <Link to={'/doctor/ongoing/details/${data.id}'} key={data.id}>
                <Card className={"ongoing-card-style"} title={title(data)} hoverable={true} bordered={true}>
                    <p className="ongoing-card-content1">{data.patient_name}</p>
                    <p className="ongoing-card-content2">{data.reason}</p>
                </Card>
            </Link>
        ));
    };
